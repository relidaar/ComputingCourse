...
# collision helpers
def group_collision(group, other):
    global explosions
    collisions = 0
...
def group_group_collision(group, other_group):
    collisions = 0
    for obj in list(group):
        if group_collision(other_group, obj):
            group.discard(obj)
            collisions += 1
    return collisions
...
def draw(canvas):
    global time, started, lives, score, rocks, explosions
    if lives <= 0:
        started = False
        rocks = set()
        explosions = set()
    ...
    process_sprites(canvas, explosions)
    process_sprites(canvas, rocks)
    process_sprites(canvas, missiles)
    ...
    score += group_group_collision(missiles, rocks)

    if group_collision(rocks, my_ship):
        lives -= 1
...
def rock_spawner():
    global rocks, rocks_limit
    if len(rocks) >= rocks_limit: return
    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
    if dist(my_ship.get_position(), rock_pos) < MIN_DISTANCE: return
